...
export default function Page() {
  ...
  return <main>
    ...
    <section className="section-shell"><div className="site-container grid gap-12 lg:grid-cols-2 lg:gap-20"><Reveal className="relative aspect-[3/2] overflow-hidden rounded-[2rem] border border-white/10"><Image src="/images/03-baslangic-gitar-dersi.webp" alt="Darende Gitar başlangıç çalışma alanı" fill sizes="(max-width: 1024px) 92vw, 48vw" className="object-cover" /></Reveal><Reveal delay={.08}><SectionHeading eyebrow="Yaklaşımımız" title="Merakı tekniğe, tekniği müziğe dönüştürmek." /><div className="mt-7 space-y-5 text-base leading-8 text-muted"><p>İyi bir başlangıcın gösterişli değil, anlaşılır olması gerektiğine inanıyoruz. Bu yüzden temel hareketleri küçük parçalara ayırıyor, öğrencinin kendi hızında güvenle ilerlemesini sağlıyoruz.</p><p>Her ders; yaşa, seviyeye, müzik zevkine ve çalışma temposuna göre şekillenir. Amacımız yalnızca akor öğretmek değil, düzenli çalışma alışkanlığı ve kalıcı bir müzik sevgisi kazandırmaktır.</p></div></Reveal></div></section>
